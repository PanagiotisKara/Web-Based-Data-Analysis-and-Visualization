...
├── data/                     # CSV data files (sensor datasets)
    ...
    ├── AirQ_imputed.csv      #AirQ.csv and AirQComp.csv with imputed Data (if there is no such file, it is generated after the first time you load the Air Quality page)
    ...
    ├── SpSo_imputed_cache.json #Cached SpSo data after imputation for faster loading (if there is no such file, it is generated after the first time you load the Speed page)
